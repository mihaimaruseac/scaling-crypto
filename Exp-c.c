...
#pragma unroll
	...
	c->limbs_used = max(a->limbs_used, b->limbs_used);
 ...
 * q = a * b >>
 */
void bn_llm(limb_t a, limb_t b, limb_t *m, limb_t *q)
{
 ...
 * c = ab*((2^64)^{-||m||}) mod m
 */
void bn_montgomery(bn *a, bn *b, bn *m, bn *c)
{
 ...
 * ab mod m = c
 */
void bn_mul(bn *a, bn *b, bn *m, bn *c)
{
	bn reductor;
 ...
 * a^b mod N
 */
void bn_exp(bn *a, bn *b, bn *m, bn *c)
{
	bn r, base;
	limb_t exp;
	int i;
...
int main(int argc, char *argv[])
{
	...
	gpuExps = (bn *)malloc(size);
	gpuResults = (bn *)malloc(size);

	/* copy vectors to device */
	memcpy(gpuExps, exps, size);
	memcpy(&N, (void*)&cpuN, sizeof(bn));
	memcpy(&Base, (void*)&cpuBase, sizeof(bn));

	/* GPU computation */
	gettimeofday(&start, NULL);
	work(gpuExps, gpuResults);
	gettimeofday(&stop, NULL);
	msecTotal = 1000 * (stop.tv_sec - start.tv_sec) + (stop.tv_usec - start.tv_usec + 0.0) / 1000;
